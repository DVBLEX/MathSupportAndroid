...
public class Formula9Fragment extends CalculationFragment {

    @BindView(R.id.formula9_n)
    TextView nText;
    @BindView(R.id.formula9_lambda)
    TextView lambdaText;
    @BindView(R.id.formula9_recycler)
    RecyclerView resultRecycler;

    private List<Double> items;

    private Formula9Adapter adapter;
    ...
    private void initUI() {
        if (items != null) {
            CalculationHelper.Formula9Calculation formula9 = CalculationHelper.getCalculationFor9Formula();
            List<Formula9Model> result = formula9.calculate(items, CalculationHelper.EXP);
            ...
            adapter = new Formula9Adapter(items, result);
        }
    ...
    public int getLayoutId() {
        return R.layout.fragment_formula9;
    }
}
